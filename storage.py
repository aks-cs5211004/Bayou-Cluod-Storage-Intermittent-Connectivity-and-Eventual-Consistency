...
class Storage:
  ...
  def apply(self, commits: list[LogEntry], tentatives: SortedList[LogEntry]) -> None:
    ...
    with self._apply_lock:
      for entry in commits:
        if entry.ltime.ts > self.c[entry.ltime.name].ts:
          entry.do(self.committed_st)
          self.committed_log.append(entry)
          self.c.advance(entry.ltime)
      
      # rollback tentative state now
      for entry in reversed(self.tentative_log):
          entry.undo(self.tentative_st)
      
      for entry in tentatives:
        if entry.ltime.ts > self.f[entry.ltime.name].ts:
          self.tentative_log.add(entry)
          self.f.advance(entry.ltime)
        
      for entry in self.tentative_log:
        entry.do(self.tentative_st)
        
      if not self.f.is_vtime_earlier(self.c): # c>f?
        # self.tentative_log = SortedList()
  ...
  def anti_entropy(self, c: VectorTime, f: VectorTime) -> tuple[list[LogEntry], SortedList[LogEntry]]:
    ...
    # TODO-4
    # Ensure the other storage's commit vector is either strictly earlier or later, not the same
    if not c.is_vtime_earlier(self.c) and not self.c.is_vtime_earlier(c):
        raise AssertionError("Commit vectors are neither strictly earlier nor later.")

    committed = [entry for entry in self.committed_log if not c.is_ltime_earlier(entry.ltime)]
    tentative = SortedList([entry for entry in self.tentative_log if not f.is_ltime_earlier(entry.ltime)], key = lambda l: l.ltime)
